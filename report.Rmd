...
data <- Cleaned %>%
  clean_names() %>%
  mutate(
    no_of_days = case_when(no_of_days == "." ~ 0,
                           TRUE ~ as.numeric(no_of_days)
  )) %>%
  group_by(ethnicity) %>%
  summarise(mean = mean(no_of_days))


 ggplot(data = data, aes(x = ethnicity, y = mean)) +
  geom_col() 
...
# Related library we need
library(tidyverse)
# Loading your data set
bat_data <- read_csv("data/bats.csv")
 
# Incomplete code snippet for the created data visualization
bat_data %>%
  ggplot(aes(x = ________, y = body_mass)) +
  ________(alpha = 0.3) +
  geom_smooth(method = ________, se = FALSE) +
  labs(
    title = "Relationship between body size traits in bats"
  )
